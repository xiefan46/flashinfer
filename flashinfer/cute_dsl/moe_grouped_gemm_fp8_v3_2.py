...
FP8 Grouped GEMM v3.2 — GEMM1 Direct FP8 Output Epilogue
==========================================================

Based on v3. Key change: GEMM1 uses the v3.2 kernel which outputs FP8 +
per-128-block scales directly from the epilogue, eliminating the separate
_quantize_output_fp8() kernel launch and BF16→FP8 memory round-trip.
...
from .moe_grouped_gemm_cutedsl_v3_2 import moe_grouped_gemm_fp8_cutedsl_v3_2
...
def moe_gemm1_fp8_v3_2(
    ...
    gemm1_out_fp8, gemm1_out_sc = moe_grouped_gemm_fp8_cutedsl_v3_2(
        a_flat,
        gemm1_weights,
        a_scale_flat,
        gemm1_weights_scale,
...
moe_gemm2_fp8_v3_2 = moe_gemm2_fp8_v3
