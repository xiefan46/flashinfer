...
CuTeDSL DeepSeek FP8 MoE Pipeline v3 — Flat Layout
====================================================

Aligned with trtllm architecture: flat 2D A/C tensors with 128-aligned
expert boundaries instead of 3D batched tensors padded to max_M.

Key difference from v2: between GEMM1 and GEMM2, data stays in flat padded
layout (total_padded_M rows). Only scatter at GEMM1 input and unsatter at
GEMM2 output. SwiGLU operates pointwise on flat layout directly.

Pipeline: Routing -> Scatter -> GEMM1(flat) -> SwiGLU -> GEMM2(flat) -> Unsatter -> Finalize
...
def cutedsl_fp8_moe_v3(
    ...
    """CuTeDSL DeepSeek FP8 MoE full pipeline v3 (flat layout).
    ...
    # --- Stage 2: GEMM1 (A-gather + scatter to flat + FP8->FP8) ---
    ...
    # Operates pointwise on flat padded layout (processes padding rows too,
    # but they are zero so SwiGLU(0)=0, no effect on final output)
    act_out, act_scale = moe_swiglu_fp8_requant(
        gemm1_out,
        gemm1_scale,
    )

    # --- Stage 4: GEMM2 (FP8->BF16, flat layout) ---
    gemm2_out_flat = moe_gemm2_fp8_v3(
        act_out,
        gemm2_weights,
        act_scale,
        gemm2_weights_scale,
