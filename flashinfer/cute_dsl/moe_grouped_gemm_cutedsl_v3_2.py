...
CuTeDSL FP8 Flat Grouped GEMM v3.2 — Direct FP8 Output Epilogue
=================================================================

Based on v3 flat grouped GEMM. Key change: the epilogue directly outputs
FP8 + per-128-block float32 scales, eliminating the separate BF16→FP8
quantization kernel and its memory round-trip.

v3 epilogue: F32 accum → BF16 → R2S → TMA store → GMEM → _quantize_output_fp8()
v3.2 epilogue: F32 accum → compute scale → FP8 quantize → direct global store

Since mma_tiler_mn=(128,128), each N tile = one 128-block. Each of 128
epilogue threads handles one M row and can independently compute the
per-(m_row, n_block) absmax without cross-thread reduction.
...
@flashinfer_api
def moe_grouped_gemm_fp8_cutedsl_v3_2(
    a: torch.Tensor,
    b: torch.Tensor,
    a_scale: torch.Tensor,
    b_scale: torch.Tensor,
    ...
    """CuTeDSL flat grouped GEMM v3.2 with direct FP8 output + per-128-block scales.

    Eliminates the separate BF16→FP8 quantization kernel by fusing FP8
    quantization into the GEMM epilogue.
