"""Tests for CuTeDSL DeepSeek FP8 MoE pipeline v3.3 (cluster_shape=(2,1)).

Validates that v3.3 produces the same output as v3.2.
"""
...
def _check_pipeline_available():
    ...
    try:
        ...
        from flashinfer.cute_dsl.moe_pipeline_v3_3 import cutedsl_fp8_moe_v3_3
...
def test_v3_3_vs_v3_2(seq_len):
    """Test CuTeDSL v3.3 pipeline output matches v3.2."""
    from flashinfer.cute_dsl.moe_pipeline_v3_2 import cutedsl_fp8_moe_v3_2
    from flashinfer.cute_dsl.moe_pipeline_v3_3 import cutedsl_fp8_moe_v3_3
    ...
    # Run v3.3 (cluster_shape=(2,1))
    v3_3_out = cutedsl_fp8_moe_v3_3(
        routing_logits, routing_bias, hidden_states, hs_scale,
        g1w, g1ws, g2w, g2ws, **common_kwargs,
    )
    ...
    v3_3_f32 = v3_3_out.float()

    cos_sim = torch.nn.functional.cosine_similarity(
        v3_2_f32.flatten(), v3_3_f32.flatten(), dim=0
    ).item()

    abs_diff = (v3_2_f32 - v3_3_f32).abs()

    print(f"\n[v3.3 vs v3.2] seq_len={seq_len}")
    print(f"  v3.2 max: {v3_2_f32.abs().max().item():.6e}")
    print(f"  v3.3 max: {v3_3_f32.abs().max().item():.6e}")
    ...
    assert v3_3_f32.abs().max().item() > 0, "v3.3 output is all zeros"
    # v3.3 is same kernel as v3.2, just different cluster shape
...
@pytest.mark.skipif(not _check_sm100(), reason="Requires SM100+")
@pytest.mark.skipif(not _check_pipeline_available(), reason="Pipeline not available")
def test_v3_3_small_batch():
    """Test v3.3 pipeline with very small batch (edge case)."""
    from flashinfer.cute_dsl.moe_pipeline_v3_3 import cutedsl_fp8_moe_v3_3
...
@pytest.mark.skipif(not _check_sm100(), reason="Requires SM100+")
@pytest.mark.skipif(not _check_pipeline_available(), reason="Pipeline not available")
def test_v3_3_output_preallocated():
    """Test v3.3 pipeline with pre-allocated output tensor (destination passing)."""
    from flashinfer.cute_dsl.moe_pipeline_v3_2 import cutedsl_fp8_moe_v3_2
    from flashinfer.cute_dsl.moe_pipeline_v3_3 import cutedsl_fp8_moe_v3_3
    ...
    assert v3_3_out.data_ptr() == output.data_ptr(), "Output not written in-place"
    assert v3_3_out.abs().max().item() > 0, "Output is all zeros"
    assert not torch.isnan(v3_3_out).any(), "Output contains NaN"

    # Verify correctness against v3.2
    v3_2_out = cutedsl_fp8_moe_v3_2(
        routing_logits, routing_bias, hidden_states, hs_scale,
        g1w, g1ws, g2w, g2ws, **common_kwargs,
    )
    cos_sim = torch.nn.functional.cosine_similarity(
        v3_2_out.float().flatten(), v3_3_out.float().flatten(), dim=0
    ).item()
    print(f"\n[v3.3 dest-passing vs v3.2] Cosine sim: {cos_sim:.6f}")
    assert cos_sim >= 0.999, f"Cosine similarity {cos_sim:.6f} < 0.999"


@pytest.mark.skipif(not _check_sm100(), reason="Requires SM100+")
@pytest.mark.skipif(not _check_pipeline_available(), reason="Pipeline not available")
def test_v3_3_gemm1_fp8_output_correctness():
    """Test GEMM1 FP8 output directly: v3.3 vs v3.2 at the GEMM1 level.

    Compares the dequantized GEMM1 outputs to verify that the cluster_shape
    change does not affect correctness.
    """
    from flashinfer.cute_dsl.moe_grouped_gemm_fp8_v3_2 import moe_gemm1_fp8_v3_2
    from flashinfer.cute_dsl.moe_grouped_gemm_fp8_v3_3 import moe_gemm1_fp8_v3_3
    ...
    # Run GEMM1 with v3.2 (cluster_shape=(1,1))
    ...
    # Run GEMM1 with v3.3 (cluster_shape=(2,1))
    v3_3_out, v3_3_scale, _, _, _, _ = moe_gemm1_fp8_v3_3(
        hidden_states, g1w, hs_scale, g1ws,
        routing_result.m_indptr,
        routing_result.permuted_idx_to_token_idx,
    )
    ...
    total_M = v3_2_out.shape[0]
    two_I = v3_2_out.shape[1]

    v3_2_deq = v3_2_out.to(torch.float32) * v3_2_scale.t().repeat_interleave(BLOCK, dim=1)[:total_M]
    v3_3_deq = v3_3_out.to(torch.float32) * v3_3_scale.t().repeat_interleave(BLOCK, dim=1)[:total_M]

    cos_sim = torch.nn.functional.cosine_similarity(
        v3_2_deq.flatten(), v3_3_deq.flatten(), dim=0
    ).item()

    print(f"\n[GEMM1 v3.3 vs v3.2] total_M={total_M}")
    print(f"  v3.2 deq max: {v3_2_deq.abs().max().item():.6e}")
    print(f"  v3.3 deq max: {v3_3_deq.abs().max().item():.6e}")
    print(f"  Cosine sim: {cos_sim:.6f}")

    assert v3_2_deq.abs().max().item() > 0, "v3.2 GEMM1 output is all zeros"
    assert v3_3_deq.abs().max().item() > 0, "v3.3 GEMM1 output is all zeros"
    assert cos_sim >= 0.999, f"GEMM1 cosine similarity {cos_sim:.6f} < 0.999"


if __name__ == "__main__":
    if _check_sm100() and _check_pipeline_available():
        test_v3_3_vs_v3_2(16)
        print("test_v3_3_vs_v3_2 passed")
        test_v3_3_small_batch()
        print("test_v3_3_small_batch passed")
        test_v3_3_output_preallocated()
        print("test_v3_3_output_preallocated passed")
        test_v3_3_gemm1_fp8_output_correctness()
        print("test_v3_3_gemm1_fp8_output_correctness passed")
    else:
